...
int main() {
    ...
    const int number_of_segments = 1000;
    std::cout << "N_THREADS: " << number_of_threads << std::endl;
    std::cout << "N_SEGMENTS: " << number_of_segments << std::endl;
    ...
    for (int i = 0; i < n_warmup; ++i) {
        ...
        sequential_warmup.seq_point_clustering(tmp_copy, number_of_segments, 14, 
    ...
    for (int i = 0; i < n_test; ++i) {
        ...
        sequential_test.seq_point_clustering(tmp_copy, number_of_segments, 14,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
        auto end = std::chrono::high_resolution_clock::now();
        sequential_duration += std::chrono::duration<double, std::milli>(end - start).count();
    }

    // warming up for parallel
    for (int i = 0; i < n_warmup; ++i) {
        std::vector<Point> tmp_copy = original_lidar_data;
        parallel_warmup.par_point_clustering(tmp_copy, number_of_segments, 14,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
    }
    ...
    for (int i = 0; i < n_test; ++i) {
        std::vector<Point> tmp_copy = original_lidar_data;
        auto start = std::chrono::high_resolution_clock::now();
        parallel_test.par_point_clustering(tmp_copy, number_of_segments, 14,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
        auto end = std::chrono::high_resolution_clock::now();
