...
int main() {
    ...
    std::cout << "N_BINS: " << number_of_bins << std::endl;
    ...
    for (int i = 0; i < n_warmup; ++i) {
        ...
        sequential_warmup.seq_point_clustering(tmp_copy, 50, number_of_bins, 
    ...
    for (int i = 0; i < n_test; ++i) {
        ...
        sequential_test.seq_point_clustering(tmp_copy, 50, number_of_bins,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
    ...
    for (int i = 0; i < n_warmup; ++i) {
        std::vector<Point> tmp_copy = original_lidar_data;
        parallel_warmup.par_point_clustering(tmp_copy, 50, number_of_bins,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
    }
    ...
    for (int i = 0; i < n_test; ++i) {
        std::vector<Point> tmp_copy = original_lidar_data;
        auto start = std::chrono::high_resolution_clock::now();
        parallel_test.par_point_clustering(tmp_copy, 50, number_of_bins,
            readLiDAR.get_max_range(), 0.3, 0.15, 0.2, 0.3);
        auto end = std::chrono::high_resolution_clock::now();
